...
\begin{chapter}{N\'umeros complejos}\label{chap-num-compl}
    ...
    \begin{section}{Multiplicación de polinomios por FFT}\label{seccion-fft}

        Como vimos en la sección \ref{seccion-definicion-polinomios} el producto de polinomios se calcula usando  que $x^ix^j = x^{i+j}$ y la propiedad distributiva. Si un polinomio tiene grado $n$ y el otro  tiene grado $m$,  entonces son necesarias $nm$ multiplicaciones de coeficientes (``todos contra todos''). 
        ...
        Este nivel de complejidad ($n^2$) parece ser razonable a nivel computacional, pero  si los polinomios a multiplicar tiene grados muy altos puede ser necesario contar con métodos más rápidos, o que requieran menos operaciones. En  esta sección mostraremos la multiplicación de polinomios usando  la transformada rápida de Fourier (FFT) y mostraremos que usando este método se puede multiplicar dos polinomios de grado $n$ en \textit{alrededor de  $n\log_2(n)$ operaciones.}  

        El  método se puede aplicar a cualquier grado, pero es mucho más fácil de explicar cuando el grado de los polinomios es una potencia de $2$. Como todo $n$ cumple que para algún $k$, $2^{k-1} < n \le 2^k$ es claro como podemos extender el método a cualquier polinomio. Por lo tanto, de ahora en más consideraremos polinomios de grado $n = 2^k$. 

        La primero observación importante es que todo polinomio de grado  $n$  está determinado por $n+1$ valores que toma.

        \begin{proposicion}
            Sa $f$ un polinomio de grado  $n$ y $x_0,\ldots,x_{n} \in \R$ todos distintos entre sí. Sea $y_i = f(x_i)$, $0 \le i \le n$. Si $g$ polinomio de grado $n$ tal que $g(x_i) = y_i$ con  $0 \le i \le n$, entonces se cumple que $g=f$.
        \end{proposicion}
        \begin{proof} Sea $h = f- g$,  es claro que $\operatorname{gr}(h) \le n$. Si $h \ne 0$, por la proposición \ref{th-pol-raiz}, $h$ tiene a lo más $n$ raíces. Sin embargo $h(x_i)= f(x_i)-g(x_i) = y_i -y_i =0$,  es decie $h$ tiene al menos $n+1$ raíces. Esto provoca un absurdo que vino de suponer que $h \ne 0$. Por lo tanto,  $h=0$ y en consecuencia $f=g$.   
        \end{proof}

    Hay muchas formas de reconstruir un polinomio de grado $n$ a partir de $n+1$ valores que toma y una de las más conocidas es el \textit{método de interpolación de Lagrange}. En  forma precisa, no es demasiado complicado probar que  
            \begin{equation*}
        ...
        Observemos que podemos calcular los valores que toma un polinomio usando el lenguaje de las matrices de la siguiente forma: 
        \begin{equation*}
            \begin{bmatrix}
