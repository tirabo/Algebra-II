...
\vskip .4cm \pause
\begin{teorema}\label{th-dim-nu-im}
...
Sea $R$ la MERF equivalente por filas a $A$. Sea $k$ la cantidad de filas de $R$ no nulas y, entonces, $n-k$ es la cantidad de variables libres de $R$. \pause


\vskip .4cm

Por el Corolario 4.4.3 de las Notas (visto en la clase 14) las filas no
nulas de $R$ forman una base del espacio de de fila, luego 
\vskip .4cm 
\begin{equation*}
    \textit{rango fila de } A = k.\tag{*}
\end{equation*}
...
\begin{frame}
    ...
    \pause
    \vskip .4cm 
Luego, por el lema anterior, $\dim\nuc(T) = n-k$.

Como por el teorema de la dimensión $\dim\K^n = n$, tenemos que 
\begin{equation*}
    \dim\nuc(T) + \dim\im(T) = n,
\end{equation*}
...
\begin{frame}
    ...
    El espacio columna de $A$ es igual a la imagen de $T$.\pause
...
&
T(x_1,\ldots,x_n) = \left[\begin{array}{cccc}
    \mid& \mid& &\mid\\
    C_1 & C_2 & \cdots &C_n\\
    \mid& \mid& &\mid
    \end{array}
...
Luego $\im(T) = \langle C_1,\ldots,C_n \rangle$.

Por lo tanto,  $\dim\im(T) =$  rango columna de $A$ $\overset{(**)}{=}$ $k$. \qed
...
\begin{frame}
   ...
   \vskip .3cm
   \begin{definicion}[(5.3.1 de las Notas)]
...
\end{definicion}
    ...
    \begin{frame}
        ...
        \begin{block}{Proposición}\label{inyectiva-sii-nuT=0}
    ...
    \begin{frame}
        \begin{observacion} Sea $T: V \to W$ transformación lineal, \pause
            \begin{enumerate}
    ...
    \begin{frame}
        ...
        \begin{proof} \pause
            \vskip -.2cm
         ($\Rightarrow$) 
           Sea $ \mathcal B$ base de $V$.  Como $T$ es isomorfismo, $T$ es mono y epi, luego por proposición de p. \ref{prop-T-mono-sii-li-2-li}, $T(\mathcal B)$ es LI y genera $W$,  es decir,  es base de $W$.

           ($\Leftarrow$) Sea $ \mathcal B$ base de $V$ y $T:V \to W$ transformación lineal tal que $T(\mathcal B)$ es base. Por lo tanto, manda un conjunto LI a un conjunto LI y un conjunto de generadores de $V$ a un conjunto de generadores de $W$. Por proposición de p. \ref{prop-T-mono-sii-li-2-li}, $T$ es mono  y epi, por lo tanto $T$ es un isomorfismo. \qed
        \end{proof}
    ...
    \begin{teorema}\label{th-equiv-epi-mono}\vskip -.2cm
    ...
    \end{definicion}
    \pause
    \begin{corolario}[(del teorema de p. \ref{th-equiv-epi-mono}) ]
...
\vskip .6cm\pause
    ...
    \begin{frame}
        ...
        \begin{itemize}
            ...
            \item $|$filas no nulas de $R|$ \;$=$\; rg-fil $A$ \;$=$\; rg-col $A$ \;$=$\; $\dim \im(A)$.\vskip .4cm\pause
