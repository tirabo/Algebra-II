...
\begin{itemize}
 \item[(i)] Una matriz con coeficientes reales puede no tener autovalores reales pero sí complejos (matriz  \ref{autovalores-6}) o tener ambos (matriz \ref{autovalores-4}).
 \item[(ii)] Para describir paramétricamente los autoespacios podemos necesitar distintas cantidades de parámetros para los distintos autovalores (la matriz \ref{autovalores-3}). Esta cantidad mínima de parámetros es lo que llamaremos {\it dimensión}.
 \item[(iii)] La cantidad de autovalores distintos es menor o igual al tamaño de la matriz. Incluso puede tener un sólo autovalor (matriz \ref{autovalores-5} y más generalmente la matriz \ref{mas-autovalores-5} del Ejercicio \ref{mas}) o tener tantos como el tamaño (matriz \ref{autovalores-2}).
\end{itemize}
...
\textbf{Observación} Algunos libros definen el polinomio característico de la matriz $A$ como $\tilde\chi_A(x)=\det(A-x\Id_n)$. Como vemos en el ejercicio anterior, ambas definiciones sirven para encontrar autovalores de $A$. El polinomio $\chi_A(x)$ tiene la particularidad de ser mónico, o sea que el coeficiente del término $x^n$ es $1$, mientras que el coeficiente del término $x^n$ de $\tilde\chi_A(x)=\det(A-x\Id_n)$ es $1$ si $n$ es par y $-1$ si $n$ es impar.
...
\ref{tr det} \ref{tr det b} Desarrollar el determinante de $x\Id-A$ por la primera columna y hacer inducción en el tamaño de la matriz. Es decir, primero
\begin{align*}
\chi_A(x) &= \det(x\Id-A)\\
