   \begin{chapter}{Producto interno}\label{chap-esp-prod-int}
    ...
    \begin{section}{Producto interno}\label{seccion-producto-interno}
        ...
        \begin{definicion}si para todo $v, w, u \in V$, se satisface:
            ...
            \la \,,\,\ra: V \times V \to \R .
            \end{equation*}
            Diremos que $\la \,,\,\ra$ es un \textit{producto interno}\index{producto interno} 
        ...
        Obviamente el producto escalar en $\R^n$  es un producto interno,  que llamaremos  el  \textit{producto interno canónico} de $\R^n$. Los resultados de esta sección valen en general para un producto interno en un $\R$-espacio vectorial de dimensión finita, pero tendremos siempre en mente el producto escalar en $\R^n$. 
        ...
        \begin{definicion}
            Sea $V$  un espacio  vectorial y $\la \,,\,\ra$ un producto interno en $V$, entonces para todo $v \in V$, definimos la \textit{norma}\index{norma} de $v$ como 
            $$
            ||v|| = \sqrt{\la v,v\ra}.
            $$
        \end{definicion}
        ...
        \begin{definicion}  Sea $V$  un $\R$-espacio vectorial con producto interno $\la \,,\,\ra$  y  sean $x,y \in V$ no nulos. Diremos que $x$ e $y$  son \textit{perpendiculares}\index{vectores perpendiculares} u  \textit{ortogonales}\index{vectores ortogonales}, y lo denotamos  $x \perp y$, si $\la x,,y\ra =0$. 
        \end{definicion}
        ...
        \begin{observacion}\label{obs-proyeccion-ort}
            No es difícil ver en un dibujo que  para vectores en $\R^2$ o $\R^3$ 
            \begin{equation*}
        ...
        \begin{proposicion}[Desigualdad de Cauchy-Schwarz]\label{prop-cauchy-schwarz} Sea $V$  un $\R$-espacio vectorial con producto interno $\la \,,\,\ra$ y sean $u, v\in V$. Entonces 
            $$|\la u, v \ra| \le ||u||||v||.$$
        \end{proposicion}
        \begin{proof} 
        ...
        \begin{teorema}[Desigualdad triangular] Sea $V$  un $\R$-espacio vectorial con producto interno $\la \,,\,\ra$ y sean $u, v\in V$, entonces 
        ...
        \begin{definicion} Sea $V$  un $\R$-espacio vectorial con producto interno $\la \,,\,\ra$. Si $X \subset V$ es ortogonal (ortonormal) y es base, diremos que $X$ es una \textit{base ortogonal} (resp. \textit{base ortonormal}) o diremos que $X$ es \textit{BO} (resp. \textit{BON}).\index{base ortogonal}\index{base ortonormal}
        \end{definicion}
        ...
        \end{proof}
        \begin{ejemplo*} 
            Si $u=(1,1)$, $v=(1,-1)$, entonces $||u||=||v|| = \sqrt{2}$ y $(\frac{1}{\sqrt{2}},\frac{1}{\sqrt{2}}),(\frac{1}{\sqrt{2}},\frac{-1}{\sqrt{2}})$ es una base ortonormal.  
            
        \end{ejemplo*}
        \vskip .2cm
        Sea $V$  un $\R$-espacio vectorial de con producto interno $\la \,,\,\ra$ ¿existe una base ortogonal de $V$? Si $\dim(V) < \infty$  la respuesta es afirmativa y esto se deducirá del proceso de ortonormalización de Gram-Schmidt que veremos a continuación. 

        El proceso de ortonormalización de Gram-Schmidt, consta de  un algoritmo que permite pasar de una base cualquiera $\{v_1,\ldots, v_n\}$ de $V$ a una base ortonormal $\{u_1,\ldots, u_n\}$, con la importante propiedad de que, para $m$ con  $1 \le m \le n$, el subespacio generado por los vectores $\{u_1,\ldots, u_m\}$ es el mismo que el subespacio generado por los vectores $\{v_1,\ldots, v_m\}$.
        
        La idea del proceso es sencillo para dos vectores: sean  $v_1,v_2 \in V$ no  nulos y no proporcionales, vimos en la observación \ref{obs-proyeccion-ort} que  los vectores
        \begin{equation*}
    ...
    \begin{section}{Operadores autoadjuntos (*)}\label{secccion-operadores-autoadjuntos}
        ...
        \begin{definicion}  Sea $T: V \to W$ una transformación lineal de rango $r$ entre espacios de dimensión finita 	con producto interno. Entonces, las raíces cuadradas de los autovalores del operador $T^*T$ son llamados los \textit{valores singulares}\index{valores singulares} de  $T$. 
        \end{definicion}
